...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'POST':
      return createParentChild();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getParentChildren() {
    const data = await prisma.parent_child
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'parent_child'));
  ...
  async function createParentChild() {
    ...
    const data = await prisma.parent_child.create({
      data: body,
    });
    return res.status(200).json(data);
  }
