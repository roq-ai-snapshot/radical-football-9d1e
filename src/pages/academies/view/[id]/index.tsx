...
import { deleteCoachAcademyById } from 'apiSdk/coach-academies';
...
function AcademyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AcademyInterface>(
    ...
    () =>
      getAcademyById(id, {
        relations: ['user', 'coach_academy', 'player_profile'],
      }),
  );

  const coach_academyHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
          ...
          <>
            ...
            {hasAccess('coach_academy', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
              <>
                ...
                <NextLink passHref href={`/coach-academies/create?academy_id=${data?.id}`}>
                ...
                <TableContainer>
                  <Table variant="simple">
                    ...
                    <Tbody>
                      {data?.coach_academy?.map((record) => (
                        <Tr key={record.id}>
                          <Td>{record.created_at as unknown as string}</Td>
                          <Td>{record.updated_at as unknown as string}</Td>
                          <Td>
                            <NextLink passHref href={`/coach-academies/edit/${record.id}`}>
                              <Button as="a">Edit</Button>
                            </NextLink>
                          </Td>
                          <Td>
                            <NextLink passHref href={`/coach-academies/view/${record.id}`}>
                              <Button as="a">View</Button>
                            </NextLink>
                          </Td>
                          <Td>
                            <Button onClick={() => coach_academyHandleDelete(record.id)}>Delete</Button>
                          </Td>
                        </Tr>
                      ))}
                    </Tbody>
                    ...
                    <Tbody>
                      {data?.player_profile?.map((record) => (
                        <Tr key={record.id}>
                          ...
                          <Td>
                            <NextLink passHref href={`/player-profiles/view/${record.id}`}>
                              <Button as="a">View</Button>
                            </NextLink>
                          </Td>
                          <Td>
                            <Button onClick={() => player_profileHandleDelete(record.id)}>Delete</Button>
                          </Td>
                        </Tr>
                      ))}
                    </Tbody>
